...
def huntflow_webhook(request):
    ...
    if event == 'APPLICANT':
        ...
        if status == 'Hired':
            ...
            # Для webhook используем системного пользователя или первого доступного
            from django.contrib.auth import get_user_model
            User = get_user_model()
            system_user = User.objects.filter(huntflow_access_token__isnull=False).first()
            
            if not system_user:
                return JsonResponse({
                    'status': 'error',
                    'message': 'No user with HuntFlow tokens found'
                })
            
            sync_service = HuntflowSyncService(system_user)
...
# HuntFlow Import Views
class HuntflowImportView(LoginRequiredMixin, TemplateView):
    """Страница импорта данных из HuntFlow"""
    template_name = 'hiring_plan/huntflow_import.html'
    
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        
        # Не загружаем данные синхронно - они будут загружены через AJAX
        context['vacancies'] = []
        context['error_message'] = None
        
        # Проверяем только токены пользователя
        if not self.request.user.huntflow_access_token:
            context['error_message'] = "У пользователя не настроен access token HuntFlow"
        elif not self.request.user.is_huntflow_token_valid:
            context['error_message'] = "Access token HuntFlow истек"
        
        return context


@login_required
def huntflow_import_applicant(request):
    """AJAX импорт конкретного кандидата"""
    ...
    try:
        ...
        if not vacancy_id or not applicant_id:
            return JsonResponse({'error': 'Missing vacancy_id or applicant_id'}, status=400)
        
        sync_service = HuntflowSyncService(request.user)
        hiring_request, message = sync_service.import_hiring_request_from_huntflow(
            vacancy_id, applicant_id, custom_data=data
        )
        ...
        if hiring_request:
            return JsonResponse({
                ...
                'message': message,
                'hiring_request_id': hiring_request.id,
                'hiring_request_url': reverse('hiring_plan:hiring_request_detail', args=[hiring_request.id])
            })
        else:
            return JsonResponse({
                'success': False,
...
@login_required
def huntflow_get_vacancies(request):
    """AJAX получение списка вакансий с кандидатами для импорта"""
    try:
        sync_service = HuntflowSyncService(request.user)
        
        # Используем новый эффективный метод
        vacancies = sync_service.get_all_importable_applicants_by_vacancy()
        ...
        return JsonResponse({
            'success': False,
            'error': f'Ошибка при подключении к HuntFlow: {str(e)}'
        })


@login_required
def huntflow_get_applicants(request, vacancy_id):
    """AJAX получение списка кандидатов для импорта по вакансии"""
    try:
        sync_service = HuntflowSyncService(request.user)
        applicants = sync_service.get_importable_applicants(vacancy_id)
