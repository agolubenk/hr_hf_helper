...
document.addEventListener('DOMContentLoaded', function() {
        ...
        window.currentNamespace = 'accounts';
...
// Функция для переключения видимости пароля
    ...
    // Определяем правильное имя поля в зависимости от провайдера
    let fieldName;
    if (provider === 'notion') {
        fieldName = 'notion_integration_token';
    } else if (provider === 'huntflow' && system) {
        fieldName = `huntflow_${system}_api_key`;
    } else {
        fieldName = `${provider}_api_key`;
    }
    
    const apiKeyField = document.getElementById(fieldName);
    const apiUrlField = system ? document.getElementById(`${provider}_${system}_url`) : null;
    ...
    if (apiUrlField) {
        data['api_url'] = apiUrlField.value;
    }
    if (system) {
        data['system'] = system;
    }
    
    fetch(`/accounts/test-${provider}/`, {
        method: 'POST',
    ...
    fetch('/huntflow/tokens/api/')
        ...
        .then(data => {
            ...
            if (data.success) {
                ...
                // Показываем только предупреждения, если токены не валидны
                if (!status.valid) {
                ...
                // Обновляем информацию под полями токенов
                const accessTokenInfo = document.getElementById('access_token_info');
                const refreshTokenInfo = document.getElementById('refresh_token_info');
                
                // Функция для определения цвета по оставшемуся времени
                function getExpirationColor(expiresAt) {
                    ...
                    if (daysLeft > 4) {
                        return 'text-success'; // Зеленый - более 4 дней
                    } else if (daysLeft >= 2) {
                        return 'text-warning'; // Желтый - 2-4 дня
                    } else {
                        return 'text-danger'; // Красный - менее 2 дней
                    }
                }
                
                if (accessTokenInfo) {
                    let accessText = 'Access token для API запросов (действует 7 дней';
                    let colorClass = '';
                    
                    if (status.access_expires_at) {
                        const expirationDate = new Date(status.access_expires_at);
                        accessText += `, истекает: ${expirationDate.toLocaleString()}`;
                        colorClass = getExpirationColor(status.access_expires_at);
                    }
                    accessText += ')';
                    
                    accessTokenInfo.textContent = accessText;
                    accessTokenInfo.className = `form-text ${colorClass}`;
                }
                
                if (refreshTokenInfo) {
                    let refreshText = 'Refresh token для обновления access token (действует 14 дней';
                    let colorClass = '';
                    
                    if (status.refresh_expires_at) {
                        const expirationDate = new Date(status.refresh_expires_at);
                        refreshText += `, истекает: ${expirationDate.toLocaleString()}`;
                        colorClass = getExpirationColor(status.refresh_expires_at);
                    }
                    refreshText += ')';
                    
                    refreshTokenInfo.textContent = refreshText;
                    refreshTokenInfo.className = `form-text ${colorClass}`;
                }
...
function testHuntflowConnection() {
    const testButton = document.querySelector('button[onclick="testHuntflowConnection()"]');
    const statusDiv = document.getElementById('huntflow-token-status');
    ...
    fetch('/huntflow/tokens/test/')
        ...
        .then(data => {
            if (data.success) {
                // Показываем уведомление о валидности токенов с данными пользователя
                const userInfo = data.user_info;
                let html = '<div class="alert alert-success"><i class="fas fa-check-circle me-2"></i>✅ Токены валидны</div>';
                
                if (userInfo && userInfo.name && userInfo.email) {
                    html += `<div class="alert alert-info">
                        <strong>Данные пользователя:</strong><br>
                        <strong>Имя:</strong> ${userInfo.name}<br>
                ...
                statusDiv.innerHTML = `<div class="alert alert-danger"><i class="fas fa-exclamation-triangle me-2"></i>❌ Ошибка подключения: ${data.error}</div>`;
            }
        })
        .catch(error => {
            console.error('Ошибка:', error);
            statusDiv.innerHTML = '<div class="alert alert-danger"><i class="fas fa-exclamation-triangle me-2"></i>❌ Произошла ошибка при тестировании подключения</div>';
        })
        .finally(() => {
            testButton.disabled = false;
            testButton.innerHTML = '<i class="fas fa-plug me-1"></i>Тест подключения';
        });
}
