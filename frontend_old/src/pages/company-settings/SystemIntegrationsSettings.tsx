...
import { toastSuccess, toastInfo } from '../../utils/toastHelper';
...
const SystemIntegrationsSettings: React.FC = () => {
    ...
    switch (action) {
      ...
      case 'delete':
        if (integration && window.confirm(`Вы уверены, что хотите удалить интеграцию "${integration.name}"?`)) {
          ...
          toastSuccess(`Интеграция "${integration.name}" удалена.`, 'Удалено');
        }
        break;
      case 'settings':
      case 'sync':
      ...
      default:
        ...
        toastInfo(`Действие '${action}' для '${integrationName}'`, 'Информация');
        break;
    }
  };
  ...
  const handleAddIntegration = (integrationToAdd: Integration) => {
    ...
    // Добавляем интеграцию в список
    setIntegrations(prev => [...prev, newIntegration]);
    toastSuccess(`Интеграция "${integrationToAdd.name}" добавлена.`, 'Успешно');
  };

  const availableIntegrations = useMemo(() => {
    const installedIds = new Set(integrations.map(i => i.id));
    // Фильтруем только те, которые еще не установлены
    return [];
  }, [integrations]);
      ...
      <div className="settings-header">
        <div className="settings-header-content">
          ...
          <div className="settings-header-actions">
            <button className="btn btn-primary" onClick={() => handleAction('add_new', '')}>
              <i className="bi bi-plus-lg me-2"></i>Добавить
            </button>
            <button className="btn btn-light" onClick={() => toastInfo('Документация по интеграциям', 'Информация')}>
              <i className="bi bi-life-preserver me-2"></i>Документация
            </button>
          </div>
        </div>
      </div>
            ...
            return (
              <div className="integration-card-v2" key={integration.id}>
                ...
                <div className="card-v2-footer">
                  <button className="card-action-btn" onClick={() => handleAction('settings', integration.id)}>
                    <i className="bi bi-gear-fill"></i> Настроить
                  </button>
                  <button className="card-action-btn" onClick={() => handleAction('sync', integration.id)} disabled={!integration.lastSync}>
                    <i className="bi bi-arrow-repeat"></i> Синхр.
                  </button>
                  <div className="dropdown">
                    ...
                    <ul className="dropdown-menu dropdown-menu-end">
                      <li><button className="dropdown-item" type="button" onClick={() => handleAction('toggle', integration.id)}>
